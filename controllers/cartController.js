...
const onlinepay = async (req, res) => {
  try {
    ...
    const newOrderId = generateOrderId();
    console.log("gen order", newOrderId);
    ...
    res.json({ razorpayOrder, orderId: order.orderId });
  } catch (error) {
...
const addToCart = async (req, res) => {
  try {
    ...
    const priceToSave = categoryOfferActive
      ? product.offerPrice
      ...
      if (totalQuantity > 5) {
        return res
          ...
          .json({
            success: false,
            message:
              "You can only add up to 5 of the same product to your cart.",
          });
      }
...
const updateCart = async (req, res) => {
  try {
    ...
    const cartItemIndex = userCart.product.findIndex(
      (item) => item.productId.toString() === productId
    );
    ...
    if (cartItemIndex !== -1) {
      ...
      const basePrice = categoryOfferActive
      ...
      if (direction === "up") {
        // Increase logic
        if (
          userCart.product[cartItemIndex].quantity < maxQuantityPerPerson &&
          userCart.product[cartItemIndex].quantity < currentProduct.countStock
        ) {
          userCart.product[cartItemIndex].quantity++;
          userCart.product[cartItemIndex].price =
            basePrice * userCart.product[cartItemIndex].quantity;
        } else if (
          userCart.product[cartItemIndex].quantity >= maxQuantityPerPerson
        ) {
          return res.json({
            error: "Cannot add more than the maximum allowed quantity",
          });
        } else {
  ...
  try {
    ...
    cartData.product.forEach((item) => {
      totalPrice +=
        ...
        item.productId.offerPrice < item.productId.productPrice
...
const loadCheckout = async (req, res) => {
  try {
    ...
    const validCoupons = availableCoupons.filter((coupon) => {
      ...
      const isUserEligible = !coupon.redeemedUsers.some(
        (user) => user.userId === String(userId)
      );
      return isMinPurchaseValid && isUserEligible;
    });
    const filteredCoupon = validCoupons.filter(
      (coupon) => !coupon.redeemedUsers.includes(userId)
    );
    ...
    res.render("users/checkout", {
      ...
      availableCoupons: filteredCoupon,
    });
  } catch (error) {
...
const placeOrder = async (req, res) => {
  try {
    ...
    if (paymentMethod === "Cash on delivery" || paymentMethod === "Wallet") {
        ...
        if (orderProduct) {
          if (orderProduct.countStock >= item.quantity) {
            ...
            return res
              .status(400)
              .json({
                message: `Insufficient stock for product: ${orderProduct.productName}`,
              });
          }
        }
      }
      ...
      if (appliedCoupon) {
        const userRedeemed = appliedCoupon.redeemedUsers.find(
...
const addCoupon = async (req, res) => {
  ...
  try {
    ...
    if (!nameRegex.test(couponName)) {
      return res.json({
        error: "Coupon name should only contain alphabets and spaces.",
      });
    }
    ...
    if (isNaN(minPurchaseValue) || minPurchaseValue < 100) {
      return res.json({
        error: "Minimum purchase amount must be at least Rs. 100.",
      });
    }
    ...
    if (
      isNaN(discountValue) ||
      discountValue <= 0 ||
      discountValue >= minPurchaseValue
    ) {
      return res.json({
        error:
          "Discount amount must be a positive number and less than the minimum purchase amount.",
      });
    }
  ...
  try {
    ...
    if (req.session.appliedCoupon) {
      return res.status(400).json({
        success: false,
        message:
          "A coupon has already been applied. Please remove it before applying another one.",
      });
    }
...
const verifyPayment = async (req, res) => {
  try {
    ...
    if (appliedCoupon) {
      appliedCoupon.redeemedUsers.push(req.session.user_id);
      appliedCoupon.save()
      req.session.appliedCoupon=null
    }
    ...
    res
      .status(500)
      .json({ message: "Something went wrong while verifying the payment" });
  }
};
...
const failedOrder = async (req, res) => {
  try {
    ...
    res.status(200).json({
      message: "order saved. Make payment after some time",
    });
  } catch (error) {
...
const retryPayment = async (req, res) => {
  try {
    ...
    console.log("order to retry", order);
    ...
    console.log("response to retry pay", razorpayOrder);

    res.status(200).json({
      success: true,
      message: "bla blabalajnna",
      razorpayOrder,
